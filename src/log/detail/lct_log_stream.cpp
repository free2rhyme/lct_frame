...
{
    std::time_t now  = CLctLogClock::to_time_t(m_timepoint);
    m_timestamp = *localtime_r(&now, &m_timestamp);
}

CLctLogStream::CLctLogStream(const std::string& loggerName,
...
const CLctLogMessageShp& CLctLogStream::messagePtr()
{
    if(m_messageShp == nullptr){
        m_messageShp.reset(new CLctLogMessage(m_sstream.str() + m_content));
    }
    return m_messageShp;
}
